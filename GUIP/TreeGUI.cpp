...
Node* Parser::statement() {
    Node* root = nullptr;
    Token error{ "ERROR", "invalid statement" };
...
Node* Parser::if_stmt() {
    Node* root = nullptr;
    Node* child;
    Token error{ "ERROR", "syntax error in if-statement" };
...
Node* Parser::repeat_stmt() {
    Node* root = nullptr;
    Node* child;
    Token error{ "ERROR", "missing until in repeat-statement" };
...
Node* Parser::assign_stmt() {
    ...
    Node* child;
    Token assign;
    Token error{ "ERROR", "invalid assign syntax" };
...
Node* Parser::read_stmt() {
    Node* root = nullptr;
    Token r;
    Token error{ "ERROR", "invalid read syntax" };
...
Node* Parser::write_stmt() {
    Node* root = nullptr;
    Node* child;
    Token error{ "ERROR", "invalid write syntax" };
...
Node* Parser::factor() {
    Token error{ "ERROR", "invalid syntax" };
...
public:
    ...
    void draw(Node* tree) {
        ...
        // Generate the PNG
        string outputPath = "SavedFiles\\tree.png";
        // Remove the existing PNG file if it exists
        if (std::remove(outputPath.c_str()) == 0) {
            std::cout << "Existing PNG file removed successfully.\n";
        }
        else {
            std::cout << "No existing PNG file to remove or failed to remove it.\n";
        }

        std::string command = "dot -Tpng tree.dot -o \"" + outputPath;
